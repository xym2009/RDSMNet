...
#from lib.Swin_Unet_networks.Load_Model_swin_Unet import load_ViT_seg as ViT_seg
...
parser = ArgumentParser(description='ResDepth:\nA Deep Prior For 3D Reconstruction From High-resolution Satellite ' 'Images (Testing)',
...
def main():
    ...
    print('input_channel: ', cfg.model.input_channels)
    ...
    for i, dataset in enumerate(cfg_data):
        ...
        if cfg.model.input_channels != 'geom':
            ...
            # dataset.images_std = 242.530 
            # dataset.images_mean = 404.575
    print("___raw")
...
# for zzg
    # images_mean, images_std = utils.compute_satellite_image_normalization(cfg_data)
    # dataset.images_mean = images_mean
    # dataset.images_std = images_std
    # dataloader1 = utils.get_dataloader(cfg_data, sampling_strategy='test',
    ...
    #                                   shuffle=False, workers=cfg.general.workers, pin_memory=False)
    # dsm_std, dsm_mean = utils.compute_local_dsm_std_per_centered_patch(dataloader1)
    # dataset.dsm_mean = dsm_mean
# for zzg end
    ...
    for index, dataset in enumerate(cfg_data):
        ...
        for p, image_pair in enumerate(image_pairs):
            ...
            print(f"程序运行时长: {elapsed_time:.6f} 秒")
            if 'raster_gt' in dataset:
                ...
                logger_index += 1
                logger_stats = utils.setup_logger(f'stats_logger{logger_index}', level=logging.INFO, log_to_console=False, log_file=file_stats)
                ...
                residuals = evaluate_performance(prediction, dataloader.dataset.dsm_input_gdal,
                                                 dataloader.dataset.raster_gt, logger,
