...
class DsmOrthoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, input_channels, tile_size, sampling_strategy, stride=None, transform_dsm=True,
        ...
        if stride is None and self.sampling_strategy == 'test':
            self.stride = int(0.5*self.tile_size)
    ...
    def __getitem__(self, index):
        ...
        # ----------------------------------------- Load DSM sample ----------------------------------------- #
        dsm_input = self.dsm_input[y:y+self.tile_size, x:x+self.tile_size]  
        if self.raster_gt is not None:
            dsm_target = self.dsm_target[y:y+self.tile_size, x:x+self.tile_size]
        else:
            dsm_target = np.nan
        # ------------------------------------------ Get loss masks ------------------------------------------ #
        if self.sampling_strategy == 'train':
            #loss_mask = self._get_dsm_loss_mask(dsm_target, self.nodata)
            loss_mask = self.mask_ground_truth[y:y+self.tile_size, x:x+self.tile_size]
            ...
            if self.raster_gt is not None:
                #loss_mask = self._get_dsm_loss_mask(dsm_target, self.nodata, patch_valid_pixels)
                loss_mask = self.mask_ground_truth[y:y+self.tile_size, x:x+self.tile_size]
            else:
                loss_mask = np.nan
        # ---------------------------------------- DSM normalization ----------------------------------------- #
        if self.transform_dsm:
            if not self.dsm_mean:
                ...
                dsm_mean = np.ma.mean(np.ma.masked_where(loss_mask == False, dsm_input))
                if type(dsm_mean) != np.float64 and type(dsm_mean) != np.float32 :
                    #print('dsm_input is not valid')
                    print(dsm_mean,type(dsm_mean))
                    dsm_mean = 10
            else:
                # Use user-specified mean
                dsm_mean = self.dsm_mean
            transform = data_normalization.get_transform(dsm_mean, self.dsm_std)
            dsm_input = transform(dsm_input)
            if self.raster_gt is not None:
                dsm_target = transform(dsm_target)
        else:
            # Add third dimension manually (grayscale image)
            # Note: transform() adds the third dimension automatically
            dsm_input = torch.from_numpy(dsm_input).unsqueeze(0) 
            dsm_mean = 0
            if self.raster_gt is not None:
                dsm_target = torch.from_numpy(dsm_target).unsqueeze(0)
        ...
        if self.input_channels != 'geom':
            ...
            #orthos = self.orthos[y:y+self.tile_size, x:x+self.tile_size, :].transpose((2, 0, 1))
    ...
    def _load_data(self):
        ...
        if self.raster_gt is not None:
            dsm_target_gdal = rasterutils.load_raster(self.raster_gt)
            self.dsm_target = dsm_target_gdal.GetRasterBand(1).ReadAsArray().astype(np.float32)
            # Save nodata value of the DSM data
            self.nodata = np.array(dsm_target_gdal.GetRasterBand(1).GetNoDataValue()).astype(np.float32)
            dsm_target_gdal = None
        else:
            self.nodata = np.array(self.dsm_input_gdal.GetRasterBand(1).GetNoDataValue()).astype(np.float32)
        if self.mask_in != None:
            ds = rasterutils.load_raster(self.mask_in)
            self.mask_ground_truth = ds.GetRasterBand(1).ReadAsArray().astype(np.uint8)
            ds = None
        else:
            ds = None
            self.mask_ground_truth = ds
    ...
    def _determine_patches(self):
        if self.sampling_strategy == 'train':
            ...
            print("Total position: ",len(valid_positions))
            ...
            print("sampled_positions: ",len(sampled_positions)) 
            sampled_positions2=sampled_positions.copy()
            #total_num_tre = self.tile_size*self.tile_size / 2
            total_num_tre = self.tile_size*self.tile_size
            for i in range(len(sampled_positions)):#mask_ground_truth
                y,x = sampled_positions[i]
                #print("i: ", i ,", y: ", y, ",x: ", x, ";en(sampled_positions): ",len(sampled_positions))
                mask_i = self.mask_ground_truth[y:y+self.tile_size, x:x+self.tile_size]
                valid_num = np.sum(mask_i)
                if (valid_num<total_num_tre):
                    list=sampled_positions[i]
                    sampled_positions2.remove(list)
            sampled_positions=sampled_positions2.copy()
            print("sampled_positions using mask: ",len(sampled_positions)) 
            self.n_samples = len(sampled_positions)
            ...
            if self.input_channels == 'geom-stereo' and len(self.image_pairs) > 1:
                if self.use_all_stereo_pairs:
                    ...
                    self.patch_position = [sampled_positions[i] for i in indices_repeat] # self.patch_position = [valid_positions[i] for i in indices_repeat]
                    # Combine each sampled DSM patch with every stereo image pair to form a training sample
                    # Example for n=3: self.image_pair_indices = [0, 1, 2, 0, 1, 2, 0, 1, 2]
                    self.image_pair_indices = np.tile(np.linspace(0, n, n, endpoint=False, dtype=np.int64), self.n_samples)
        ...
        # Get a regular grid of patches without overlap
            ...
            positions, patch_valid_pixels = rasterutils.create_regular_grid(self.area_defn, tile_size=self.tile_size, stride=self.stride)
            #print("------self.area_defn: ",self.area_defn)
            #print('------val positions len: ',len(positions))
            #print('------val patch_valid_pixels len: ',len(patch_valid_pixels))
            positions2=positions.copy()
            patch_valid_pixels2=patch_valid_pixels.copy()
            total_num_tre = self.tile_size*self.tile_size / 2
            for i in range(len(positions)):
                ...
                valid_num = np.sum(mask_i)
                if (valid_num<total_num_tre):
                    positions2.remove(positions[i])
                    patch_valid_pixels2.remove(patch_valid_pixels[i])
            positions=positions2.copy()
            patch_valid_pixels=patch_valid_pixels2.copy()
            print('val positions: ',len(positions))
            print('val patch_valid_pixels: ',len(patch_valid_pixels))
            ...
            if self.input_channels != 'geom':
                ...
                self.image_pair_indices = np.repeat(np.linspace(0, n, n, endpoint=False, dtype=np.int64), len(positions))
        ...
        # Get a regular grid of patches with overlap
            ...
            #print('test patch_valid_pixels: ',len(patch_valid_pixels))
    ...
    def _verify_inputs(self, dataset):
        ...
        if 'raster_in' not in dataset:
            print("ERROR: Missing argument 'raster_in' in 'dataset'. Specify the path of the input depth/height " "raster.\n")
            sys.exit(1)
        ...
        if self.sampling_strategy in ['train', 'val'] and 'raster_gt' not in dataset:
            print("ERROR: Missing argument 'raster_gt' in 'dataset'. Specify the path of the ground truth depth/height " "raster.\n")
            sys.exit(1)
        ...
        if self.input_channels in ['geom-multiview', 'geom-stereo', 'geom-mono', 'stereo']:
            ...
            if 'image_list' not in dataset:
                print("ERROR: Missing argument 'image_list' in 'dataset'. Specify a list of paths to the precomputed " "ortho-rectified images.\n")
            ...
            if 'image_pairs' not in dataset:
                ...
                if self.sampling_strategy == 'test':
                    if len(dataset.image_pairs) > 1:
                        raise ValueError("ERROR: Invalid argument 'image_pairs' in 'dataset'. " "Specify a single image pair.\n")
                n_images_per_pair = []
        ...
        if 'area_defn' not in dataset or 'x_extent' not in dataset.area_defn or \
            ...
            if not isinstance(x_extent, list) or not isinstance(y_extent, list):
                raise ValueError("ERROR: Invalid argument 'x_extent' or 'y_extent' in 'dataset'. " "Enter a list of tuples.\n")
        ...
        if self.input_channels in ['stereo', 'geom-stereo'] and len(dataset.image_pairs[0]) != 4:
