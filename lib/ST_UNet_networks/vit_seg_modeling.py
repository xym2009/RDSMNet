...
from .vit_seg_modeling_resnet_skip import TransResNetV2,TransResNetV2_im_trans
...
class Transformer(nn.Module):
    def __init__(self, config, img_size):
        ...
        #self.hybrid_model = TransResNetV2_im_trans(config, block_units=config.resnet.num_layers, width_factor=config.resnet.width_factor) #swin部分只输入左图+右图+左右图差异，unet主体输入原始三通道x
...
class DecoderCup(nn.Module):
    def __init__(self, config):
       ...
       # print("in_channels,out_channels, skip_channels:",in_channels,out_channels, skip_channels)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('111', x.shape)
        x_0 = x[:, 0, :, :]  # 输入dsm wxq
        x_0 = x_0.unsqueeze(1)  # 输入dsm wxq

        x, features = self.transformer(x)  # (B, n_patch, hidden)

        TT = torch.tensor(0) # wxq debug nan
        TT = TT.to(x.device)  # wxq debug nan
        if (torch.equal(x,TT)): # wxq debug nan
            #print("TTTTTT")
            logits=x_0 # wxq debug nan
        else:
            #print('222', x.shape)
            x = self.decoder(x, features)
            logits = self.segmentation_head(x)

            logits = add(x_0, logits)  # wxq 残差
        return logits
