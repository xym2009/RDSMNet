...
from lib.ST_UNet_networks.deform_conv import DeformConv2d
...
class MixStyle(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("mu_perm:",mu_perm.shape)
        #print("mu:",mu.shape)
        mu_mix = mu * interpolation + mu_perm * (1 - interpolation) #interpolation：lambda
...
class TransResNetV2(nn.Module):
    ...
    config.trans.depths = [2, 2, 6, 2] '''
        ...
        #self.encoder = Encoder(config) #transformer
        #self.mixStyleModule = MixStyle(p=0.5, alpha=0.1) #wxq
    ...
    def forward(self, x):
      ...
      #  print('www',x.shape)
        ...
        for i in range(len(self.body)):
            #print("before body:",x.shape)
            x0=x
            x = self.body[i](x)
            if (math.isnan(x.min())|math.isnan(x.max())):  # wxq debug nan
                #print("x min:",x.min(),", x0 min:",x0.min())  # wxq debug nan
                return torch.tensor(0), torch.tensor(0)  # wxq debug nan
            #if math.isnan(x.max()):
            #    print("x min:",x.max(),", x0 max:",x0.max())
           # print("i:",i,",x:",x.min(),', ',x.max())
            #print("after body:",x.shape)
            #if (i<len(self.body)-1):
              #  print("before mixStyle:",x.shape)
            #    x = self.mixStyleModule(x) #wxq
               # print("after mixStyle:",x.shape)

            x = self.Dcn[i](x)
          #  print("after Dcn:",x.shape)
            s1=self.Conv3[i](Trans_features[i])
            short = self.Fuse[i](s1)
            short = short * x
...
class TransResNetV2_im_trans(nn.Module):  #swin部分只输入左图+右图+左右图差异，unet主体输入原始三通道x
    """Implementation of Pre-activation (v2) ResNet mode.#self.hybrid_model = Res
    img_size=224, patch_size=4, in_chans=3, embed_dim=96,NetV2((3,4,9),1)"""
    ...
    def __init__(self, config, block_units, width_factor):
        ...
        #                              embed_dim=embed_dim,
        #                              norm_layer=nn.LayerNorm if self.patch_norm else None)
        self.embeddings = PatchEmbed(img_size=config.data.img_size, patch_size=8, in_chans=config.data.in_chans-1,
                                     embed_dim=embed_dim,
                                     norm_layer=nn.LayerNorm if self.patch_norm else None)  
    ...
    def forward(self, x):
        ...
        im_lr = (left+right- torch.abs(left-right))/2
        #trans_x=torch.cat([left, right,im_lr], dim=1)
        trans_x=torch.cat([left, right], dim=1)
        del left, right,im_lr
