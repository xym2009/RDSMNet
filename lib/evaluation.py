...
def compute_residuals(raster, raster_gt, nodata, mask_gt=None):
    ...
    print("raster_masked:",raster_masked.shape)
    print("raster_gt_masked:",raster_gt_masked.shape)
...
def evaluate_performance_disp(basename, prediction, dsm_input_gdal, dsm_target_gdal, eva_outfile):
    #build eva_outfile 
    if not os.path.isdir(os.path.dirname(eva_outfile)):
        os.makedirs(os.path.dirname(eva_outfile))
    #load dsm gt
    if isinstance(dsm_target_gdal, gdal.Dataset):
        dsm_target_gdal = dsm_target_gdal.GetRasterBand(1).ReadAsArray().astype(np.float64)
    #mask1 = (dsm_target_gdal > -60.0) #zzg
    extrapath = "/workspace/resdepth/sv1101/mask.mat"
    extramask = scio.loadmat(extrapath)['x']
    mask1= extramask!=-999.0#sd sv1101
    mask3= (dsm_target_gdal != -999.0)
    mask2 = (dsm_target_gdal != np.nan)
    mask = np.logical_and(mask3,mask2)
    # mask = np.logical_and(np.logical_and(mask1,mask2),mask3)
    errormap = np.abs(prediction[mask] - dsm_target_gdal[mask])
    abs_mae = np.mean(errormap)
    abs_rmse = np.sqrt(np.mean(np.multiply(errormap, errormap)))
    rel_mae = np.mean(errormap- np.median(errormap))
    rel_rmse = np.sqrt(np.mean(np.multiply(errormap- np.median(errormap),errormap- np.median(errormap))))
    with open(eva_outfile, 'a') as f:
        towrite = 'abs MAE:'+str(abs_mae)+' abs RMSE: '+str(abs_rmse) +' rel MAE:'+str(rel_mae) +' rel RMSE: '+str(rel_rmse)
        f.write(basename)
        f.write('\n')
        f.write(towrite)
        f.write('\n')
    return abs_mae, abs_rmse, rel_mae, rel_rmse
...
def evaluate_performance(raster_prediction, ds_raster_input, ds_raster_gt, logger_root, area_defn=None,
    ...
    if path_gt_mask:
        if not fdutil.file_exists(path_gt_mask):
            ...
            ds = rasterutils.load_raster(path_gt_mask)
            mask.ground_truth = ds.GetRasterBand(1).ReadAsArray().astype(np.bool)
            ds = None
    else:
    ...
    # print("----------------data.initial:",type(data.initial))
    # dsm_input_gdal = rasterutils.load_raster("/mnt/wxq/train_ST_Unet_using2/train/raw_stunet_newmask_mixstyle_notCF/output_directory/2024-03-09_18-16_ResDepth-stereo/SD_TEST/OMH_dxre_matchtoGT.tif")
    # dsm_input = dsm_input_gdal.GetRasterBand(1).ReadAsArray().astype(np.float32)
    # data.initial=dsm_input
    # dsm_input_gdal = rasterutils.load_raster("/mnt/wxq/train_ST_Unet_using2/train/raw_stunet_newmask_mixstyle_notCF/output_directory/2024-03-09_18-16_ResDepth-stereo/SD_TEST/OMH_isprs_gdal_Clip.tif")
    # dsm_input = dsm_input_gdal.GetRasterBand(1).ReadAsArray().astype(np.float32)
    # data.prediction=dsm_input

    # print("----------------data.initial:",residual_threshold)
    ...
    if 'building' in mask:
        ...
        stats.before.building = get_statistics(np.ma.masked_array(residuals.before.all, mask=~mask.building), residual_threshold)
        ...
        stats.before.terrain = get_statistics(np.ma.masked_array(residuals.before.all, mask=~mask.terrain), residual_threshold)
...
def predict_linear_blend_unetres(datanow, model):
    """
    Applies the model to the initial DSM patches stored in the dataloader and returns the refined DSM patches.
    The refined DSM patches are then merged by linearly blending the refined heights in overlapping patch regions.
    ...
    model.eval()
    model.to(device)
    # if dataloader.dsm_input_gdal != None:
    #     dataloader.dataset = dataloader
    # Initialize the refined DSM raster
    cols = datanow.dsm_input_gdal.RasterXSize
    rows = datanow.dsm_input_gdal.RasterYSize
    raster_out = np.zeros((rows, cols))

    tile_size = datanow.tile_size
    stride = datanow.stride

    with torch.no_grad():
        for index, batch in enumerate(datanow):
            # Extract the offset of the samples w.r.t. the entire raster
            offsets_x = batch['patch_offset_x']
            offsets_y = batch['patch_offset_y']

            # Extract the region per patch that does not overlap with any other patch (i.e., no linear blending for
            # those pixels)
            ulx = batch['patch_valid_pixels_ulx']
            uly = batch['patch_valid_pixels_uly']
            lrx = batch['patch_valid_pixels_lrx']
            lry = batch['patch_valid_pixels_lry']

            # Inference
            batch['input'] = batch['input'].unsqueeze(0)
            y_pred = model(batch['input'].to(device)).cpu()
            #y_pred,res = model(batch['input'].to(device)) #wxq
            #y_pred=y_pred.cpu() #wxq
            y_pred = data_normalization.denormalize_numpy(y_pred, batch['dsm_mean'], batch['dsm_std'])

            # each sample (DSM patch) within the batch
            # Upper-left image coordinate of the i.th patch
            x = offsets_x
            y = offsets_y

            # Get blending weights
            weights = _get_blend_weights(tile_size, stride, ulx, uly, lrx, lry)

            # Perform blending
            weighted_tile = y_pred[0, 0,:, :] * weights
            raster_out[y:y+tile_size, x:x+tile_size] += weighted_tile
