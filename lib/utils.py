...
from lib.ST_UNet_networks.vit_seg_modeling import VisionTransformer as ViT_seg0
from lib.ST_UNet_networks.vit_seg_modeling import CONFIGS as CONFIGS_ViT_seg
...
def allocate_area(cfg):
    ...
    for i, dataset in enumerate(cfg.datasets):
        if dataset.allocation_strategy == 'entire': # Use the entire raster
            extent = rasterutils.get_raster_extent(dataset.raster_in) #cfg.datasets[i]
            if 'train' in dataset.area_type:
                dataset.area_defn_train = edict({'x_extent': [(0, extent['cols'] - 1)], 'y_extent': [(0, extent['rows'] - 1)]})
            if 'val' in dataset.area_type:
                dataset.area_defn_val = edict({'x_extent': [(0, extent['cols'] - 1)], 'y_extent': [(0, extent['rows'] - 1)]})
            if 'test' in dataset.area_type:
                dataset.area_defn = edict({'x_extent': [(0, extent['cols'] - 1)], 'y_extent': [(0, extent['rows'] - 1)]})
        else: # divide the entire into three sets
            crossval_training = dataset.crossval_training if 'crossval_training' in dataset else False
            train_area, val_area, test_area = data_allocation.allocate_data(dataset.raster_in, dataset.allocation_strategy, test_stripe=dataset.test_stripe, crossval_training=crossval_training)
            if 'train' in dataset.area_type:
...
def compute_local_dsm_std_per_centered_patch(dataloader, raster_identifier='raster_in'):
    ...
    for i, batch in enumerate(dataloader):
        if raster_identifier == 'raster_in':
            dsmx = batch['input'][:, 0, :, :].numpy().astype(np.float128)
        else:
            dsmx = batch['target'][:, 0, :, :].numpy().astype(np.float128)
        mask = batch['loss_mask'][:, 0, :, :].numpy()
        dsmx = np.ma.masked_where(mask == False, dsmx)
        mean_per_sample = dsmx.mean(axis=(1, 2), keepdims=True)
        if dsmx.count()>1:
            stds[i] = np.sqrt(((dsmx - mean_per_sample) ** 2).sum() / (dsmx.count() - 1))
            means[i] = mean_per_sample
        #print(i,mean_per_sample,stds[i])
    ...
    meanv = means[np.logical_and(stds >= perc5, stds <= perc95)].mean().item()
    return std, meanv
...
def compute_satellite_image_normalization(cfg_data):
    ...
    for i, dataset in enumerate(cfg_data):
        ...
        ds = rasterutils.load_raster(dataset.mask_ground_truth)
        mask = ds.GetRasterBand(1).ReadAsArray().astype(np.bool)
        ds = None
        for index in image_ids:
            ...
            for j in range(len(area_defn.x_extent)):
                ...
                patchmask = mask[y[0]:y[1] + 1, x[0]:x[1] + 1]
                patch = np.ma.masked_where(patchmask == False, patch)
                #print(patch)
                list_data.append(patch.flatten())
...
def get_dataloader(cfg_data, sampling_strategy, transform_dsm, transform_orthos, use_all_stereo_pairs,
    ...
    list_dsets = []
    i = 0
    # Iterate over each dataset configuration % we have three traning sets in our exps sd/zzg/sa
    for dataset in cfg_data: # cfg_data 
    ...
    if len(list_dsets) > 1:
        ...
        dsets = dset
    dataloader = torch.utils.data.DataLoader(dsets, batch_size=batch_size, shuffle=shuffle, num_workers=workers, pin_memory=pin_memory)
    return dataloader
...
def get_loss(cfg, logger=None):
    ...
    elif cfg.training_settings.loss == 'L2':
        criterion = torch.nn.MSELoss()
    elif cfg.training_settings.loss == 'SmoothL1':
...
def get_model(cfg, logger=None):
    ...
    if args_model.name == 'UNet':
        print("**args_model.settings:",args_model.settings)
    ...
    # config_vit.n_skip = args.n_skip
        in_channels = 3
        #print("config_vit: ",config_vit)
        model = ViT_seg0(config_vit, img_size=256, num_classes=config_vit.n_classes)
    else:
        if logger:
...
def get_optimizer(cfg, model, logger=None):
    ...
    #print("cfg.optimizer.learning_rate:",cfg.optimizer.learning_rate)
    #print("cfg.optimizer.weight_decay:",cfg.optimizer.weight_decay)
    if cfg.optimizer.name == 'Adam':
        optimizer = torch.optim.Adam(model.parameters(), lr=cfg.optimizer.learning_rate,
                                     weight_decay=cfg.optimizer.weight_decay)
    elif cfg.optimizer.name == 'SGD':
        optimizer = torch.optim.SGD(model.parameters(), lr=cfg.optimizer.learning_rate,momentum=0.9,
                                    weight_decay=cfg.optimizer.weight_decay)
    else:
        if logger:
...
def get_scheduler(cfg, optimizer, logger=None):
    ...
    if cfg.scheduler.enabled:
        ...
        if name == 'ReduceLROnPlateau':
            ...
            print("**cfg.scheduler.settings:",cfg.scheduler.settings)
...
def prepare_dataset_configuration(cfg, phase):
    ...
    if phase == 'test':
        ...
        if cfg.model.input_channels == 'geom':
            ...
            keys = ['name', 'raster_gt', 'raster_in', 'image_list', 'mask_ground_truth']
        cfg_list = []
        # Iterate over each dataset configuration
        for i, dataset in enumerate(cfg.datasets):
            cfg_dataloader = edict()
...
def setup_logger(logger_name, level=logging.INFO, log_to_console=True, log_file=None):
    ...
    formatter = LeveledFormatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
