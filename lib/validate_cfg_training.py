...
def augment_dataset_args(cfg_user):
    ...
    unwanted = ['name', 'path_image_list', 'path_pairlist_training', 'path_pairlist_validation', 'crossval_training', 'mask_ground_truth']
...
def _valid_dataset_args(cfg_user, logger):
    ...
    for i, dataset in enumerate(cfg_user.datasets):
        ...
        if not all_keys_known(dataset, arguments.DATASET_KEYS_MANDATORY_train + arguments.DATASET_KEYS_OPTIONAL, logger):
            error[i] = True
        # Verify that the initial depth/height raster (initial DSM) exists
        if 'raster_in' not in dataset:
            logger.error("Missing argument 'raster_in'. Specify the path of the initial depth/height raster " "(initial DSM).\n")
            error[i] = True
        ...
        if 'raster_gt' not in dataset:
            logger.error("Missing argument 'raster_gt'. Specify the path of the ground truth depth/height raster " "(ground truth DSM).\n")
            error[i] = True
        ...
        if input_config in ['geom-multiview', 'geom-stereo', 'geom-mono', 'stereo']:
            ...
            if 'path_image_list' not in dataset:
                logger.error("Missing argument 'path_image_list'. Specify a text file which stores the paths of " "the ortho-rectified images.\n")
                error[i] = True
            elif not is_string(dataset.path_image_list, 'path_image_list', logger):
                error[i] = True
            elif fdutil.file_extension(dataset.path_image_list) != '.txt':
                logger.error("Invalid value for the argument 'path_image_list'. Specify a text file which stores the " "paths of the ortho-rectified images.\n")
                error[i] = True
                ...
                for path in image_paths:
                    if not fdutil.file_exists(path):
                        ...
                        error[i] = True
            print('----area_type:',dataset.area_type)
        ...
        if 'n_training_samples' in dataset and not is_positive_integer(dataset.n_training_samples, 'n_training_samples', logger):
            error[i] = True
        ...
        print("n_crossval_datasets:",n_crossval_datasets)
        if error[i]:
            logger.info('\n')
