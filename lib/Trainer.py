...
class Trainer(object):
    def __init__(self, args):
        ...
        print(self.tboard_log_dir)
        self.logger = utils.setup_logger('train_logger', level=logging.INFO, log_to_console=True, log_file=self.log_file)
        self.start_epoch = 0
    ...
    def inference_one_batch(self, batch, phase):
        ...
        if phase == 'train':
            ...
            x = x.cuda() #x = x.to(self.device)
            y = y.cuda() #y = y.to(self.device)
            loss_mask = loss_mask.cuda() #loss_mask = loss_mask.to(self.device)
            y_pred = self.model(x) # wxq
            #y_pred, LR_res = self.model(x) ## LR_res：通道0：左图，通道1：右图
            ...
            #left_res = LR_res[:,0,:,:].unsqueeze(1)
            #right_res = LR_res[:,1,:,:].unsqueeze(1)
            #print("loss: ",loss)
            #loss2 = self.criterion(left_res, right_res)
            #loss = 0.3*loss2+0.7*loss
            if loss==0.0:
            ...
            with torch.no_grad():
                ...
                #     print("npnan-mean: ",mean,", std: ", std)
                #     print("loss_mask.sum():",loss_mask.sum())
                    #np.savetxt("loss_mask.txt",y.cpu())
                #left_res = LR_res[:,0,:,:].unsqueeze(1)
                #right_res = LR_res[:,1,:,:].unsqueeze(1)
                #print("loss: ",loss)
                #loss2 = self.criterion(left_res, right_res)
    ...
    def inference_one_epoch(self, epoch, phase):
        ...
        for c_iter, batch in enumerate(self.loader[phase]):
            ...
            for key, value in stats.items():
                if math.isnan(value):
                    print("-----value: ", batch['loss_mask'].sum())
                    stats_meter[key].update(stats_meter[key-1])
                else:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.n_epochs):
            ...
            #print("----epoch: ",epoch,",self._get_lr()1: ",self._get_lr())
            # ---------------------------------------------------------------------------- #
            # TRAINING
            # ---------------------------------------------------------------------------- #
            ...
            # VALIDATION
            # ---------------------------------------------------------------------------- #
            print("---self.evaluate_rate:",self.evaluate_rate)
            print("epoch:",epoch)
            ...
            if (epoch + 1) % self.evaluate_rate == 0:
                ...
                #print("----epoch: ",epoch,",self._get_lr(): ",self._get_lr())
                # Save the best model
            ...
            #print("----epoch: ",epoch,",self._get_lr()2: ",self._get_lr())
        ...
        self.writer.close()
        # Save the last model
        self._save_checkpoint(epoch, train_stats_meter['MAE_metric'].avg, val_stats_meter['MAE_metric'].avg, self.path_model_last)
