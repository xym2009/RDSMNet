...
parser = ArgumentParser(description='ResDepth:\nA Deep Prior For 3D Reconstruction From High-resolution Satellite ''Images (Training)', formatter_class=RawFormatter)
...
def main():
    ...
    # logger.info('\nfinished get_dataloader\n-------------------\n')
    # dsm_std, dsm_mean = utils.compute_local_dsm_std_per_centered_patch(dataloader)
    # logger.info('\nfinished compute_local_dsm_std_per_patch\n-------------------\n')
    ...
    print("\ncfg_traindata: ",cfg_traindata)
    print("\ncfg_valdata: ",cfg_valdata)
    ...
    if cfg.model.input_channels in channels:
        cfg.output.satellite_image_normalization = os.path.join(cfg.output.output_directory, 'Image_normalization_parameters.p')
        logger.info(f"Writing satellite image normalization parameters to file:" f"\n{cfg.output.satellite_image_normalization}\n")
        io_control_file.write_normalization_params_to_file(cfg.output.satellite_image_normalization, images_mean, images_std)
   ...
   # model = ACC_UNet(n_channels=cfg.model.input_channels,n_classes=1,n_filts=32) # test ACC_Unet
     #model = ACC_UNet_Lite(n_channels=cfg.model.input_channels,n_classes=1,n_filts=32) # test ACC_UNet_lite
     #model = model.cuda()
   ...
   # model = VAN()
    ...
    if cfg.output.plot_model_txt:
        ...
        utils.write_model_structure_to_file(file, model, cfg.training_settings.tile_size, cfg.model.input_channels)

    # Get the optimizer, the loss function, and the learning rate scheduler
    #print("cfg.scheduler.settings: ",cfg.scheduler.settings)
    #cfg.scheduler.settings.step_size = 20
    ...
    # print("----------schedulers gamma:", scheduler.gamma)
    # print("----------schedulers step_size:", scheduler.step_size)
    # ------------------------------------------- Training ------------------------------------------- #
